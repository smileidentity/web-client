(() => {
  ...
  (function productSelection() {
    ...
    function getIdTypeName(country, id_type, productConstraints, legacyProductConstraints) {
      ...
      if (!validIdType) {
        const legacyValidIdType = legacyProductConstraints.doc_verification[country].id_types[id_type] || legacyProductConstraints.enhanced_kyc[country].id_types[id_type];
    ...
    function transformIdTypesToVerificationMethodMap(idTypes, productConstraints, legacyProductConstraints) {
      return idTypes.reduce(
        (idSelectionMap, { country, id_type, verification_method }) => {
          idSelectionMap[country] = idSelectionMap[country] || {
            name: productConstraints.find((item) => item.country.code === country)?.country?.name,
            id_types: {}
          };
      ...
      async (event) => {
        if (event.data) {
          if (event.data.includes("SmileIdentity::Configuration")) {
            ...
            const [productConstraints, legacyConstraints] = await Promise.all(constraintsPromises);
