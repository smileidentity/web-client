describe('totpConsent', () => {
  ...
  describe('core', () => {
    it('should not show up for non-BVN_MFA id types', () => {
      ...
      cy.getTotpConsentApp().should('not.be.visible');
    });
    ...
    it('should show up for BVN_MFA id types', () => {
      ...
      cy.getTotpConsentApp().find('#id-entry').should('be.visible');
    });
    ...
    describe('queryOtpModes', () => {
      it('should display a validation error message when the input is invalid', () => {
        ...
        cy.getTotpConsentApp().find('#id_number').type('000000');

        cy.getTotpConsentApp().find('#query-otp-modes').click();

        cy.getTotpConsentApp().find('#id_number-hint').should('be.visible');
      });
      ...
      it('should display a network error message when there is a network error', () => {
        ...
        cy.getTotpConsentApp().find('.validation-message').should('be.visible');
      });
    });
    ...
    describe('selectOtpMode', () => {
      it('should display a network error message when there is a network error', () => {
        ...
        cy.getTotpConsentApp().find('[type="radio"]').check('sms');
      ...
      it('should close if the user does not have access to any of the contact methods', () => {
        ...
        cy.getTotpConsentApp().find('#contact-methods-outdated').click();
    ...
    describe('submitOtp', () => {
      it('should display a network error message when there is a network error', () => {
        ...
        cy.getTotpConsentApp().find('#totp-token').type('123456');
      ...
      it('should navigate to the selectOtpMode screen when the user chooses to try another contact method', () => {
        ...
        cy.getTotpConsentApp().find('.try-another-method').should('be.visible');
  ...
  describe('for basic kyc', () => {
    ...
    it('should navigate to the id form after totp consent is granted, without the need to re-enter the id-number', () => {
      ...
      cy.getIFrameBody().find('#id-number').should('not.be.visible');
    });
    ...
    it('should set "Bank Verification Number (with OTP)" in the id selection list when previewBVNMFA is true', () => {
      ...
      cy.getIFrameBody().find('#id_type').should('contain', 'with OTP');
      ...
      cy.getIFrameBody().find('#id_type').should('not.contain', 'with OTP');
    });
  });
  ...
  describe('for biometric kyc', () => {
    ...
    it('should navigate to the camera screen, and submit after images are captured', () => {
      ...
      cy.getIFrameBody().find('end-user-consent').should('not.be.visible');
