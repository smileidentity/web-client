    ...
    try {
      ...
      if (isIOS) {
        ...
        if (gl && 'getParameter' in gl) {
          ...
          // eslint-disable-next-line no-console
          console.log('[MediaPipe] WebGL vendor:', webGLContext.getParameter(webGLContext.VENDOR));
          // eslint-disable-next-line no-console
          console.log('[MediaPipe] WebGL renderer:', webGLContext.getParameter(webGLContext.RENDERER));
        }
        ...
        if (window.WebAssembly) {
          try {
            ...
            const wasmTest = new WebAssembly.Module(new Uint8Array([0, 97, 115, 109, 1, 0, 0, 0]));
            // eslint-disable-next-line no-console
      ...
      try {
        ...
        try {
          ...
          // eslint-disable-next-line no-console
          console.log('[MediaPipe] ✅ FaceLandmarker created with CPU delegate (fallback)');
        } catch (cpuError) {
          console.error('[MediaPipe] Both GPU and CPU delegates failed:', cpuError);
          throw cpuError;
        }
      }
