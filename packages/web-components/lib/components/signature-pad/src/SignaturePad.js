...
class SmartFileUpload {
  ...
  static async retrieve(files) {
    if (files.length > 1) {
      throw new Error(t('signaturePad.error.multipleFiles'));
    }
    ...
    if (file.size > SmartFileUpload.memoryLimit) {
      throw new Error(
        tHtml('signaturePad.error.fileTooLarge', {
          filename: file.name,
...
`;
    ...
    uploadControl.innerHTML = `
      <p part="upload">
        <strong>${t('signaturePad.or')}</strong>
        <label>
          ...
          <span>${t('signaturePad.uploadLabel')}</span>
        </label>
      </p>
    `;
